        ...
        switch (args[0]) {
            case "search":
                ...
                if (user) {
                    ...
                    if (infUser) {
                        const infractionEmbed = new discord_js_1.RichEmbed();
                        const filteredInfs = infs.filter((i) => i.guild === msg.guild.id);
                        ...
                        const reactionCollector = embedMessage.createReactionCollector(filter);
                        let stopTimer;
                        const setStopTimer = () => {
                        ...
                        const forwardEmoji = "▶";
                        const basketEmoji = "🗑";
                        const botReactionBackward = await embedMessage.react(backwardEmoji);
                        const botReactionForward = await embedMessage.react(forwardEmoji);
                        reactionCollector.on("collect", (r) => {
                            switch (r.emoji.name) {
                                case forwardEmoji:
                                    setStopTimer();
                                    const forwardUserReaction = r.message.reactions.filter(r => r.emoji.name === forwardEmoji);
                                    ...
                                    embedMessage.edit(infractionEmbed);
                                    break;
                                case backwardEmoji:
                                    setStopTimer();
                                    const backwardUserReaction = r.message.reactions.filter(r => r.emoji.name === backwardEmoji);
                                    ...
                                    break;
                                case basketEmoji:
                                    const basketUserReaction = r.message.reactions.filter(r => r.emoji.name === basketEmoji);
