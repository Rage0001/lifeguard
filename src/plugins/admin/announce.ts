...
export const command = new Command(
  ...
  async (msg, args, bot) => {
    try {
      ...
      const messageCollector = (confirmationMsg as Message).createReactionCollector(
        filter,
        { time: 15000 }
      );
      ...
      messageCollector.on("collect", r => {
        switch (r.emoji.name) {
          case "✅":
            const yesUserReaction = r.message.reactions.filter(
              r => r.emoji.name === "✅"
            );
            ...
            break;
          case "❌":
            const noUserReaction = r.message.reactions.filter(
              r => r.emoji.name === "❌"
            );
