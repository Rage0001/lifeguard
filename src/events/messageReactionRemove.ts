...
export const event = new Event("messageReactionRemove", async (bot, reaction: MessageReaction, user: User) => {
  try {
    ...
    const starboardMessage = await removeStar(reaction.message.guild.id, reaction.message.id);
